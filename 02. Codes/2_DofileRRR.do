...
global dofile "C:\Users\frans\Dropbox\Projet d'économétrie\03. Codes"
global Table   "C:\Users\frans\Dropbox\Projet d'économétrie\04. Results\Tables"
...
drop ipriv_rppp
drop GDP_rppp
keep country isocode  year invest
drop if year <1990
ren isocode iso 
...
gen logc = log(credit_priv)


**Global**
global control inf trade fd fdi logc gov_stab gdpgrowthannual ka_open
										  

**column 1 OLS **
xtreg invest remitofgdp $control i.year, fe robust 

estadd local nobs="1459"
estadd local fe="Oui"
estadd local ft="Oui" 
estadd local R2= " 0.31"
...
estadd local fe="Oui"
estadd local ft="Non"
estadd local fstat="67.920" 
estadd local pval="0.000"	
estadd local ar="[.158, .673]"
estadd local R2 = " 0.19"
...
estadd local fe="Oui"
estadd local ft="Oui"
estadd local fstat="72.174" 
estadd local pval="0.000"	
estadd local ar="[.156, .777]"
estadd local R2= "0.21"
...
estadd local nobs="1354"
estadd local fe="Oui"
estadd local ft="Oui"
estadd local fstat="70.619" 
estadd local pval="0.000"	
estadd local ar="[.175, .915]"
...
global control2 inf trade fd fdi logc corup gdpgrowthannual ka_open

*Anderson Rubin Confidence Interval : On récupère les intervales de confiance de Anderson Rubin qui pemret de juger également de la force des instruments

ivreg2 invest (remitofgdp= IV2) $control2 yr* dummy*, robust first 
rivtest, ci level(90) /*.167, .811*/
...
estadd local fe="Oui"
estadd local ft="Oui"
estadd local fstat="69.086" 
estadd local pval="0.000"	
estadd local ar="[.167, .811]"
estadd local R2= "0.20"
...
*column 6 : Pour les pays ayant une bonne qualité institutionnelle (if govslevel==1).
							
*Anderson Rubin Confidence Interval : On récupère les intervales de confiance de Anderson Rubin qui pemret de juger également de la force des instruments. 

ivreg2 invest (remitofgdp= IV2) $control yr* dummy* if govslevel ==1, robust first
rivtest, ci level(90) /*.393, 1.205*/
...
estadd local fe="Oui"
estadd local ft="Oui"
estadd local fstat="35.094" 
estadd local pval="0.000"	
estadd local ar="[.393, 1.205]"
estadd local R2= "0.22"

eststo het1 

*On récupère juste le coefficient et sa p-value pour insérer dans la table des résultats. 
xtreg remitofgdp IV2 $control yr* if e(sample) & govslevel ==1, fe robust
estadd local nobs="742"
estadd local fe="Oui"
estadd local ft="Oui"
estadd local R2= "0.18"
eststo hc1


*colum 7 : Pour les pays ayant une mauvaise qualité institutionnelle (if govslevel==0). 
							
*Anderson Rubin Confidence Interval : On récupère les intervales de confiance de Anderson Rubin qui pemret de juger également de la force des instruments. 

ivreg2 invest (remitofgdp= IV2) $control yr* dummy* if govslevel ==0, robust first
rivtest, ci level(90) /*-.046,  .929*/

qui ivreg2 invest (remitofgdp= IV2),  first robust

xtivreg2 invest (remitofgdp= IV2)  $control yr*  if e(sample) & govslevel== 0, fe robust first savefirst savefprefix(fsc1) 

estadd local nobs="715"
estadd local fe="Oui"
estadd local ft="Oui"
estadd local fstat="41.259" 
estadd local pval="0.005"	
estadd local ar="[-.046, .929]"
estadd local R2= "0.25"
...
*Panel A : Résultats 2nd étape*	
		...
		"$Table\Table1.tex", fragment booktabs se(3) b(3) star starlevels(* 0.10 ** 0.05 *** 0.01)  replace  ///
				drop( _cons yr*) order(remitofgdp trade fd fdi inf ka_open logc gov_stab corup gdpgrowthannual ) varlabel( remitofgdp "TFM/(%PIB)" trade "Ouverture commerciale" fd "Développement financier" fdi "IDE" inf "Inflation" ka_open "Ouverture financière" logc "Log_crédit privé" gov_stab "Stabilité politique" corup "Contrôle de Corruption" gdpgrowthannual "Taux de croissance (PIB/tête)"  ) ///
...
********************************************************************************
	...
	esttab rc1 rc2 robc1 robc2 hc1 hc2  using ///
		"$Table\Table3.tex", fragment booktabs se(3) b(3) star starlevels(* 0.10 ** 0.05 *** 0.01)  replace  ///
				drop( _cons yr*) order( IV2 trade fd fdi inf ka_open logc gov_stab corup gdpgrowthannual ) varlabel(IV2 "IV (Instrument)" trade "Ouverture commerciale" fd "Développement financier" fdi "IDE" inf "Inflation" ka_open "Ouverture financière" logc "Log_crédit privé" gov_stab "Stabilité du gouvernement" corup "Contrôle de Corruption" gdpgrowthannual "Taux de croissance (PIB/tête)"  ) ///
				mlabel(none) style(tex)  nolines nogaps nonumbers  ///
...
/* On teste les canaux en 2 temps : on régresse d'abord le canal (FD) sur la variable d'intérêt (remitofgdp) et ensuite on régresse la variable expliquée (invest) sur le canal (FD), pas besoin d'effets fixes temporels, juste une régression bivariée */


*Canal et variable d'intérêt 


xtreg fd remitofgdp, fe ro 
estadd local nobs = "2,105"
estadd local R2 ="0.02"  
eststo can1  

*Variable expliquée et canal 
xtreg  invest fd, fe ro  	
estadd local nobs = " 2,180"
estadd local R2 ="0.02"
eststo can2 

esttab can1 can2 using ///
		"$Table\Table4.tex", fragment booktabs se(3) b(3) star starlevels(* 0.10 ** 0.05 *** 0.01)  replace  ///
				drop( _cons) order(remitofgdp fd ) varlabel( remitofgdp "TFM/(%GDP)" fd "Développement financier" ) ///
				mlabel(none) style(tex)  nolines nogaps nonumbers  ///
				stats(nobs r2, fmt(0 2) ///
				label("\hline N.Obs" "R2")) postfoot( \hline)
